<template>
  <div class="back">
    ...
    <div class="back--right" v-if="this.$route.query.status == 'new'">
      <button @click="showModal(2)">Arizani tavsiya etmaslik</button>
      <button @click="showModal(1)">Arizani tavsiya etish</button>
    </div>
    <a-modal
      ...
      :title="
        isCheck === 1
          ? 'Arizani tavsiya etish'
          : 'Arizani tavsiya etmaslik'
      "
      @cancel="handleCancel"
      :footer="null"
    >
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          this.$store.dispatch("entity/form", {
            ...
            name: "all",
            method: "put",
            url: `api/expert/updateApp/${app.id}`,
            params: {
              p: "not",
              extra: {
                status: this.isCheck == 1 ? 'Tavsiya etildi' : 'Tavsiya etilmadi',
                message: values.text
              }
            },
            cb: {
